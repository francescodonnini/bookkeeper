...
public class BufferedChannelWriteTest {
    ...
    private static Stream<Arguments> newInputs() {
        return Stream.of(
                ...
                Arguments.of(10, 0, 0, 0, "rw-rw-r--", setOf(StandardOpenOption.WRITE), EMPTY),
                Arguments.of(20, 10, 10, 0, "rw-rw-r--", setOf(StandardOpenOption.WRITE), EMPTY),
                Arguments.of(20, 10, 15, 0, "rw-rw-r--", setOf(StandardOpenOption.WRITE), EMPTY),
                Arguments.of(20, 10, 5, 0, "rw-rw-r--", setOf(StandardOpenOption.WRITE), EMPTY)
        );
    }
    ...
    public void parametricSingleWriteTest(
        ...
        if (initialFileContent.length == 0) {
            path = TemporaryFile.create("file", permissions);
        } else {
            path = TemporaryFile.create("file", permissions, initialFileContent);
        }
        ...
        try (FileChannel channel = FileChannel.open(path, options);
            ...
            if (shouldFlush(writableBytes, writeCapacity, unpersistedByteBound)) {
                ...
                Assertions.assertEquals(0, b.getUnpersistedBytes(), testFailed(summary));
                checkUnderlyingFile(path, (int) oldFileChannelPosition, flushedBytes, src);
            } else {
